...
사용자는 로컬 PC에서 Lapidary 서버를 실행하고, Tailscale을 통해 개인 네트워크 안에서 웹으로 접속한다. 모바일, 태블릿, 다른 PC에서도 tailnet에 연결된 기기라면 Obsidian 문서를 열람하고 수정할 수 있다.
...
- Tailscale tailnet 접근 제어를 전제로 안전하게 접근한다.
...
7. vault root 밖의 파일에는 절대 접근할 수 없다.
8. 모바일 브라우저에서도 사용할 수 있다.
9. Tailscale 환경에서 안정적으로 동작한다.
...
2. vault 폴더 트리에서 문서를 찾는다.
3. Markdown preview로 내용을 확인한다.
4. 필요하면 편집 모드로 전환해 수정한다.
5. 저장한다.
...
3. 내용을 수정한다.
4. 저장한다.
5. 로컬 PC의 Obsidian vault에 바로 반영된다.
